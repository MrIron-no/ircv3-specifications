...
### Capabilities
The `draft/resume-0.6` capability is advertised by servers that support resuming connections.
...
Capability negotiation example:
    ...
    C: CAP LS
    S: CAP * LS :multi-prefix draft/resume-0.6
    C: CAP REQ :draft/resume-0.6
    S: CAP * ACK :draft/resume-0.6
...
The first step in successfully resuming a connection is for the 'old client' to negotiate the `draft/resume-0.6` capability and receive a resume token. This process is illustrated here:
...
### SASL and Accounts
Servers MAY enforce a policy that the client authenticates with SASL before resuming, so that the resume token can be matched against the network account of the session being resumed. If this policy is enforced and the client sends `RESUME` before completing SASL authentication, the server MUST reject the request with `FAIL RESUME SASL_FIRST`. The client can then authenticate and retry `RESUME` during the same registration. This policy MUST be enforced only at `RESUME` time: servers MUST NOT refuse to advertise or negotiate `draft/resume-0.6` because the client has not negotiated `sasl`, since the `sasl` capability may not be available at that point (for example if services are not linked) and may be offered later via `CAP NEW`. If the client has authenticated but the account does not match that of the session identified by the token, the server MUST reject the request with `INVALID_TOKEN`, so as not to reveal that the token is valid for another account.
